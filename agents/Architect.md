...
- Write implementation plans as markdown files to `plans/{FeatureName}.md`
...
- Never write C# code, proto files, or configuration files. **Do** write plan files to `plans/{FeatureName}.md`
...
| ArticleHub | Read model — aggregated latest state from all services | PostgreSQL | Carter (partial) |
